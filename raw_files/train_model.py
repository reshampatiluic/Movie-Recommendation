...
def load_data(filepath):
    ...
    try:
        ...
        df = pd.read_csv(
            filepath,
            names=["Timestamp_x", "User_ID", "Movie_Name", "Timestamp_y", "Rating"],
            skiprows=1,
        )
...
def compute_user_similarity(utility_matrix, max_users=5000):
    ...
    if len(utility_matrix) > max_users:
        ...
        print(
            f"Sampled users from {original_size} to {len(utility_matrix)} for similarity computation"
        )
    ...
    similarity_df = pd.DataFrame(
        similarity, index=utility_matrix.index, columns=utility_matrix.index
    )
...
def evaluate_model(utility_matrix, similarity_df, test_df):
    ...
    for _, row in test_df.iterrows():
        ...
        if (
            user_id in utility_matrix.index
            and user_id in similarity_df.index
            and movie_name in utility_matrix.columns
        ):
            predicted_rating = predict_rating(
                user_id, movie_name, utility_matrix, similarity_df
            )
    ...
    prediction_coverage = (
        (successful_predictions / total_test_samples) * 100
...
def get_popular_movies(df, n=20):
    ...
    movie_stats = df.groupby("Movie_Name").agg({"Rating": ["mean", "count"]})
    ...
    movie_stats = movie_stats.sort_values(
        ["count", "avg_rating"], ascending=[False, False]
    )
...
def calculate_model_size_and_memory(df, utility_matrix, similarity_df):
    ...
    # Calculate file size
    model_file_size = os.path.getsize("data/final_data.csv") / (
        1024 * 1024
    )  # Size in MB

    # Calculate memory usage
    utility_matrix_memory = utility_matrix.memory_usage(deep=True).sum() / (
        1024 * 1024
    )  # MB
    similarity_matrix_memory = similarity_df.memory_usage(deep=True).sum() / (
        1024 * 1024
    )  # MB
...
def train_model(filepath, model_path="movie_recommender_model.pkl"):
    ...
    print(
        f"Loaded {len(df)} ratings from {len(df['User_ID'].unique())} users on {len(df['Movie_Name'].unique())} movies"
    )
    ...
    model_size_metrics = calculate_model_size_and_memory(
        df, utility_matrix, similarity_df
    )
    ...
    print(
        f"   Utility Matrix Memory: {model_size_metrics['Utility_Matrix_Memory_MB']:.2f} MB"
    )
    print(
        f"   Similarity Matrix Memory: {model_size_metrics['Similarity_Matrix_Memory_MB']:.2f} MB"
    )
