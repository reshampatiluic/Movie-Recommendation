...
def main():
    parser = argparse.ArgumentParser(
        description="Run online evaluation of the recommendation system"
    )
    parser.add_argument(
        "--visualize", action="store_true", help="Generate visualizations"
    )
    parser.add_argument(
        "--telemetry_dir", default="telemetry", help="Path to telemetry data directory"
    )
    parser.add_argument(
        "--output_dir",
    ...
    metrics = MetricsComputer(
        telemetry_dir=args.telemetry_dir, output_dir=args.output_dir
    )
    ...
    print("-" * 40)
    print(
        f"Users with recommendations: {data_summary.get('users_with_recommendations', 0)}"
    )
